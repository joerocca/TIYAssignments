...
#import "CalculatorBrain+Extras.m"
...
- (IBAction)squaredButton:(UIButton *)sender;
...
//    float returnValue = [brain useOperator];
    ...
    if ((brain.operatorType == OperatorTypeDivision) && [brain.operand2String isEqualToString:@"0"])
    {
...
- (IBAction)squaredButton:(UIButton *)sender
{
    ...
    self.displayLabel.text = [NSString stringWithFormat:@"%g", [brain squaredCalculation]];
...
- (IBAction)squareRoot:(UIButton *)sender
{
    ...
    self.displayLabel.text = [NSString stringWithFormat:@"%g", [brain squareRootCalculation]];
